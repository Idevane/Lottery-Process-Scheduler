...
uint32 my_timer_get() {
	return totalQuanta * 100 + totalQuanta * 1;
}
...
ProcessLotteryWinner() {
  PCB	*pcb;
  int i;
  //First lets get a valid winning ticket number
...
ProcessSchedule ()
{
  ...
  //Code block for round robin scheduling
  if (FALSE) {
  ...
  //BEGIN lottery scheduling block
  else if (TRUE) {
	printf("Entering lottery scheduling algorithm\n");
	currentPCB->p_quanta++;
	totalQuanta++;
...
//p_nice is process number
//p_info is total number of lottery tickets 
